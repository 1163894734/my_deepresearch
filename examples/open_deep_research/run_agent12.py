...
你是长文本报告的章节撰写智能体。
任务开始时先调用 `get_section_sop` 读取章节写作 SOP，然后严格按 SOP 执行。
你每次只写当前段落，必须读取 `state["writer_state"]` 里的 `current_section`、`outline`、`previous_section_content` 和 `available_citations`。
写完后必须调用 `citation_validation` 工具校验当前段落，再返回 JSON。
最终输出必须是 JSON，对象至少包含 `content`、`citations_used`、`final_score`、`section_title`。
"""

FINALIZATION_INSTRUCTIONS = """
你是长文本报告的整合智能体。
任务开始时先调用 `get_finalization_sop` 读取整合 SOP，然后严格按 SOP 执行。
你只负责摘要、参考文献和最终 Markdown 拼装。
读取 `state["writer_state"]` 里的 `full_content`、`written_sections`、`outline` 和 `available_citations`。
最终输出必须是 JSON，对象至少包含 `abstract_text`、`references_text`、`final_markdown`、`finalization_metadata`。
"""
...
def build_long_writer_agents(model, shared_tools, text_webbrowser_agent):
    ...
    return planning_agent, section_agent, finalization_agent
...
def create_long_writer_agent():
    ...
    planning_agent, section_agent, finalization_agent = build_long_writer_agents(
        model=model,
        shared_tools=shared_tools,
        text_webbrowser_agent=text_webbrowser_agent,
    )
    ...
    keywords_search_agent = CodeAgent(
        model=model,
        tools=shared_tools,
        max_steps=20,
        verbosity_level=2,
    ...
    long_writer_agent = LongWriterAgent(
        ...
        citation_validation_tool=citation_validation_tool,
