...
class FragmentedInfoAggregatorTool(Tool):
    ...
    def _llm_enrich_empty_fields(self, record: dict, current_idx: int, total: int) -> dict:
        import time
        import ast
        import json
        
        source_text = str(record.get("摘要（中英文）", "")) or str(record.get("关键观点/判断", "")) or str(record.get("abstract", ""))
        ...
        missing_fields = [f for f in target_fields if not record.get(f) or str(record.get(f)).strip() == ""]
        
        if len(missing_fields) < 2:
            return record
            
        print(f"   ⏳ [{current_idx}/{total}] 正在深度补全: {title[:20]}... 缺失 {len(missing_fields)} 个字段")

        prompt = f"""你是顶级科技情报分析专家。请仔细阅读以下科技文献/专利/新闻的标题和摘要，从中精准抽取信息，以填补数据表中的空缺字段。
        
【标题】{title}
【内容】{source_text}

请提取以下缺失信息的字段：{json.dumps(missing_fields, ensure_ascii=False)}

要求：
1. 必须输出合法的 JSON 格式。
2. 语言需高度浓缩概括，每个字段不超过两句话。
3. 如果原文内容中实在推导不出某个字段，请将该字段的值设为 "原文暂未明确提及"。绝不要自己瞎编。
...
*(注：你可以直接删掉 `_rule_based_maturity_and_trend` 和 `_merge_sensing` 这两个方法)*

**替换 `_build_overall_conclusion` 方法：**
```python
