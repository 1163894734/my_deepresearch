...
class BodyWritingComponent(JsonWorkflowComponent):
    ...
    输入 JSON 示例:
    {
        ...
        "available_citations": {"Chain-of-Thought Prompting Elicits Reasoning in Large Language Models": {"authors": "Wei et al.", "year": "2022", "title": "Chain-of-Thought Prompting Elicits Reasoning in Large Language Models"}},
        "task": "（用户任务描述）",
...
def main() -> int:
    ...
    if len(sys.argv) <= 1:
        ...
        prev_section_content = pre_section_path.read_text(encoding="utf-8") if pre_section_path.exists() else ""
